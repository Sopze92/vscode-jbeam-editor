...
export const 
  CFG_BACKGROUND_COLOR= 0,
  CFG_FLOOR_COLOR= 1

const localConfig= Array(2)

export async function init(){

  // Ambient light affects all objects in the scene globally.
  ambLight= new THREE.AmbientLight(0xffffff, 0.5) // soft white light
  ...
  scene.add(ambLight)
  dirlights= [keyLight, fillLight, rimLight, bottomLight]

  for(let light of dirlights) {
    light.target.position.set(0, 0, 0)
    scene.add(light)
  }

  // Create a floor
  floor = new THREE.Mesh(new THREE.PlaneGeometry(256, 256), new THREE.MeshStandardMaterial({ roughness: 1.0, metalness: .0 }))
...
export function dispose() {
  for(l of dirlights) scene.remove(l)
...
function onLocalConfigChanged() {
  ...
  renderer.setClearColor(colorScale(localConfig[CFG_BACKGROUND_COLOR], localConfig[CFG_BACKGROUND_COLOR].float32[3]).hex24)
  floor.material.color.set(localConfig[CFG_FLOOR_COLOR].hex24)
  floor.material.opacity= localConfig[CFG_FLOOR_COLOR].float32[3]
...
export function onConfigChanged() {
  ...
  localConfig[CFG_BACKGROUND_COLOR]= parseColor(ctx?.config?.sceneView?.view?.colorBackground ?? "#aaaaaa")
  localConfig[CFG_FLOOR_COLOR]= parseColor(ctx?.config?.sceneView?.view?.colorFloor ?? "#808080")
